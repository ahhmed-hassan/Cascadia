...
class ScoringService(private val rootService: RootService) : AbstractRefreshingService() {
    ...
    companion object {
        ...
        private fun depthFirstConnectedComponentLength(
            ...
            var connectedComponentLength = 1
         ...
         */
        private fun calculateElkScoreRuleA(
            i: Int, elkCoordinate: MutableSet<Pair<Int, Int>>,
            directions: List<Pair<Int, Int>>, coordinate: Pair<Int, Int>
        ): Int {
            var points = 0;
         ...
         * @return points of a player
         */
        private fun calculateElkScoreRuleB(
            i: Int, coordinate: Pair<Int, Int>,
            elkCoordinate: MutableSet<Pair<Int, Int>>
        ): Int {
            var points = 0;
         ...
         */
        private fun calculateHawkScoreRuleB(
            notAdjacent: MutableSet<Pair<Int, Int>>,
            hawkCoordinate: MutableSet<Pair<Int, Int>>
     ...
     * @param habitat The [Player] having this longest terrains
    ...
    fun calculateLongestTerrain(searchedTerrain: Terrain, habitat: MutableMap<Pair<Int, Int>, HabitatTile>): Int {
        ...
        val buildSearchedTerrainGraph:
                    (Map<Pair<Int, Int>, TileAndCoordinate>) -> Map<Pair<Int, Int>, List<Pair<Int, Int>>> =
     ...
     * @param habitat the player whose score should be calculated
     ...
     *
     * @param habitat the person you want to add the score to
     */
     ...
     * @param habitat the [Player] to calculate its runs.
    ...
    fun calculateSalmonScore(habitat: MutableMap<Pair<Int, Int>, HabitatTile>): Int {
        val makeSalmonGraph: (Map<Pair<Int, Int>, HabitatTile>) -> Map<Pair<Int, Int>, List<Pair<Int, Int>>> =
            { habitatTile ->
                ...
                val graph = salmonCoordinates.associateWith { coordinate ->
                    coordinate
                        .neighbours()
                        .filter { neighbour -> salmonCoordinates.contains(neighbour) }
                }//filter out non-salmons
                //All salmon nodes that has more than two salmon neighbours or can reach such a node
                val hasMoreThanTwoOrCanReachIt = mutableSetOf<Pair<Int, Int>>()
                for (salmonCoordinate in graph.filterValues { it.size > 2 }.keys)
                    if (!hasMoreThanTwoOrCanReachIt.contains(salmonCoordinate))
                        depthFirstConnectedComponentLength(graph, hasMoreThanTwoOrCanReachIt, salmonCoordinate)
        ...
        else mapOf(1 to 2, 2 to 5, 3 to 8, 4 to 12, 5 to 16, 6 to 20, 7 to 25)
        val maxRuns = if (isB) 5 else 7
        var salmonRuns = 0
     ...
     *
     * @param habitat the player for witch the score should be calculated
     */
    fun calculateFoxScore(habitat: MutableMap<Pair<Int, Int>, HabitatTile>): Int {
        ...
        foxes.forEach {
            ...
            if (game.ruleSet[Animal.FOX.ordinal]) {
                ...
                pairs = animals.filter { it >= 2 }.size
                ...
                differentAnimals = animals.filter { it >= 1 }.size
